    ...
    return loading ? (
        ...
        <div className="bg-black min-h-full">
            <div className="p-4 grid grid-cols-2 gap-4">
                <div className="col-span-full bg-emerald-950/40 border border-emerald-700 rounded-md p-4 text-emerald-200">
                    <FormattedMessage
                        id="today_uploaded"
                        values={{
                ...
                <div className="bg-slate-900 rounded-md p-4 flex flex-col gap-2 border border-slate-800">
                        ...
                        {(data!['play_rank'] as []).length > 0 ? (
                            (data!['play_rank'] as []).map((v, k) => (
                                <div key={k} className="flex gap-2">
                                    ...
                                    <Link to={`/z/page/movie/detail/${v['target']}`} className="underline text-slate-200">
                                        {v['title']}
                                    </Link>
                                </div>
                            ))
                        ...
                        {(data!['alive_ranking'] as []).length > 0 ? (
                            (data!['alive_ranking'] as []).map((v, k) => (
                                <div key={k} className="flex gap-2">
                                    ...
                                    <Link to={`/z/page/user/${v['user_id']}`} className="underline flex justify-between flex-1 text-slate-200">
                                        <div>{v['user_id']}</div>
