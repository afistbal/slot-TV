/** 与选集网格一致：`vip !== 0` 且 `locked === 1` 表示 VIP 锁集 */
export function isEpisodeVipLocked(ep: { vip: number; locked: number } | undefined | null) {
    if (ep == null) {
        return false;
    }
    return ep.vip !== 0 && ep.locked === 1;
}
...
export function formatTagText(value: string) {
    return value
        .replace(/[_-]+/g, ' ')
        .trim()
