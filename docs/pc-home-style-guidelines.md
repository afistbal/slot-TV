...
- 所有 PC 改动必须放在 `@media (min-width: 768px)` 内。
- `<= 767px` 的规则、尺寸、布局、动效保持原样。
...
- PC：优先与 `--app-vw` 的 480 基准等比视觉一致，必要时再在 `@media (min-width: 768px)` 内做细调。
...
- 375 / 390 宽度：与改造前视觉一致。
- 768 / 1024 宽度：进入 PC 全宽展示（与 JS `useMinWidth768` 一致）。
- 首页 Banner、书架横滑、Footer、TopNav 在 PC 下无错位、无溢出、无字体突变。

## 6) TopNav（PC 对标）补充

- `reelshort-topnav` 的 PC 改动只允许放在 `@media (min-width: 768px)`。
...
- H5（`< 768px`）维持原有行为，不切换为 PC hover 浮层交互。
